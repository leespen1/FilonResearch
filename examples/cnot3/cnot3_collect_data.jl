    ...
    for k in min_power:max_power
        ...
        if run_hermite
            ...
            data, _ = produce_or_load(config, experiment_dir;
                ...
                t0 = time()
                hist = eval_forward(rot_vec_prob, rot_controls, pcof, order=2*(config["s"]+1))
                elapsed = time() - t0
                Dict("hist" => Matrix{ComplexF64}(hist), "elapsed" => elapsed)
            end
            push!(h_hists, data["hist"])
            push!(nsteps_h, ns)
            push!(h_times, data["elapsed"])
        end
        ...
        if run_filon_zero
            ...
            push!(f0_hists, data["hist"])
            push!(nsteps_f0, ns)
            push!(f0_times, data["elapsed"])
        end
        ...
        push!(fr_hists, data["hist"])
        push!(nsteps_fr, ns)
        push!(fr_times, data["elapsed"])

        # Print progress line
        print(@sprintf("  nsteps=%-8d", ns))
        if run_hermite;    print(@sprintf("  |u|_H=%8.2e", norm(h_hists[end][:,end])));  end
        if run_filon_zero; print(@sprintf("  |u|_F0=%8.2e", norm(f0_hists[end][:,end]))); end
        print(@sprintf("  |u|_Fr=%8.2e", norm(fr_hists[end][:,end])))
        println()
    end
...
# ============================================================
# Pre-compute plot data (control functions + state evolution)
# ============================================================
...
hist_fine = Matrix{ComplexF64}(eval_forward(rot_vec_prob, rot_controls, pcof, order=order_finest))
t_fine = collect(range(0, Tmax, length=size(hist_fine, 2)))

# ============================================================
# Save aggregated summary for plotting
# ============================================================

summary_file = joinpath(experiment_dir, "summary.jld2")
