...
max_power = 6      # cap at 2^12 = 4096 timesteps
...
Δt = 100 /8
Sp_r, Sm_r = filon_S_plus_S_minus(A_matrix_funcs, frequencies_rot, 0, Δt, s)
Sp_0, Sm_0 = filon_S_plus_S_minus(A_matrix_funcs, frequencies_zero, 0, Δt, s)
Gr = Sp_r \ Sm_r
G0 = Sp_0 \ Sm_0
@show cond(Gr) maximum(abs, eigvals(Gr))
@show cond(G0) maximum(abs, eigvals(G0))

#=
for k in min_power:max_power
    ns = 2^k

    Tmax_extended = Tmax * (ns / base_nsteps)

    # Filon (rotating frame frequencies)
    hist = filon_solve(A_deriv_funcs, u0_vec, frequencies_rot, Tmax_extended, ns, s)
    ...
    if length(fr_hists) >= 2
        ...
        println("nsteps = $ns, |u| = $(norm(hist[:,end]))")
    end
end
