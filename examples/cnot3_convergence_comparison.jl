...
# ============================================================
# Problem setup (from bspline_test.jl)
# ============================================================
...
# Set carrier wave frequencies to zero (so Filon can work better)
Cfreq .= 0 
...
initial_cond = 4
u0_vec = U0[:, initial_cond]

# Frequency vectors
frequencies_zero = zeros(div(rot_prob.real_system_size, 2))
...
# Storage: s => (nsteps_list, [final_state_vectors])
hermite_data = Dict{Int, Tuple{Vector{Int}, Vector{Vector{ComplexF64}}}}()
filon_zero_data = Dict{Int, Tuple{Vector{Int}, Vector{Vector{ComplexF64}}}}()
filon_rot_data = Dict{Int, Tuple{Vector{Int}, Vector{Vector{ComplexF64}}}}()
...
for s in s_values
    ...
    println("="^100)
    println(@sprintf("  %-8s  %-14s %-6s %-14s %-6s %-14s %-12s %-12s %-12s %s",
        "nsteps", "Rich(H)", "p(H)", "Rich(Fr)", "p(Fr)", "|H-F0|", "|u|_H", "|u|_F0", "|u|_Fr", "time"))
    println("  " * "-"^130)

    h_finals = Vector{ComplexF64}[]
    f0_finals = Vector{ComplexF64}[]
    fr_finals = Vector{ComplexF64}[]
    nsteps_h = Int[]
    nsteps_f0 = Int[]
    nsteps_fr = Int[]
    ...
    for k in min_power:max_power
        ...
        hist = eval_forward(
            ...
            rot_controls, pcof, order=order)
        push!(h_finals, Vector{ComplexF64}(hist[:,end]))
        push!(nsteps_h, ns)

        # Filon (zero frequencies)
        hist = filon_solve(A_deriv_funcs, u0_vec, frequencies_zero, Tmax, ns, s)
        push!(f0_finals, Vector{ComplexF64}(hist[:,end]))
        push!(nsteps_f0, ns)

        # Filon (rotating frame frequencies)
        hist = filon_solve(A_deriv_funcs, u0_vec, frequencies_rot, Tmax, ns, s)
        push!(fr_finals, Vector{ComplexF64}(hist[:,end]))
        push!(nsteps_fr, ns)

        elapsed = time() - t_start

        # Richardson errors and convergence orders
        if length(h_finals) >= 2
            h_rich_val = norm(h_finals[end] - h_finals[end-1]) * 2^order / (2^order - 1)
        ...
        if length(fr_finals) >= 2
            fr_rich_val = norm(fr_finals[end] - fr_finals[end-1]) * 2^order / (2^order - 1)
        ...
        cross_str = fmt(norm(h_finals[end] - f0_finals[end]))

        # Norms of final state vectors
        norm_h_str  = @sprintf("%12.6e", norm(h_finals[end]))
        norm_f0_str = @sprintf("%12.6e", norm(f0_finals[end]))
        norm_fr_str = @sprintf("%12.6e", norm(fr_finals[end]))

        # Elapsed time
        time_str = elapsed < 60 ? @sprintf("%.1fs", elapsed) : @sprintf("%.1fm", elapsed/60)

        println(@sprintf("  %-8d  %-14s %-6s %-14s %-6s %-14s %-12s %-12s %-12s %s",
            ns, h_rich_str, h_ord_str, fr_rich_str, fr_ord_str, cross_str, norm_h_str, norm_f0_str, norm_fr_str, time_str))
    end

    hermite_data[s] = (nsteps_h, h_finals)
    filon_zero_data[s] = (nsteps_f0, f0_finals)
    filon_rot_data[s] = (nsteps_fr, fr_finals)
end
...
function print_richardson(label, nsteps_list, finals, order)
    println(@sprintf("\n  %-12s  %-22s  %-12s", "nsteps", "$label Rich. err", "rate"))
    for i in 1:length(nsteps_list)-1
        # Richardson error estimate for u_{nsteps_list[i]}: ||u_{2N} - u_N|| * 2^p/(2^p-1)
        diff = norm(finals[i+1] - finals[i])
        err = diff * 2^order / (2^order - 1)
        if i >= 2
            prev_diff = norm(finals[i] - finals[i-1])
...
function print_error_vs_ref(label, nsteps_list, finals, ref, order)
    println(@sprintf("\n  %-12s  %-22s  %-12s", "nsteps", "$label error", "rate"))
    prev_err = NaN
    for i in 1:length(nsteps_list)
        err = norm(finals[i] - ref)
        if err == 0.0
            continue
        end
...
for s in s_values
    ...
    h_nsteps, h_finals = hermite_data[s]
    f0_nsteps, f0_finals = filon_zero_data[s]
    fr_nsteps, fr_finals = filon_rot_data[s]

    h_ref = h_finals[end]
    f0_ref = f0_finals[end]
    fr_ref = fr_finals[end]
    ...
    print_richardson("Hermite", h_nsteps, h_finals, order)
    print_richardson("Filon(w=0)", f0_nsteps, f0_finals, order)

    print_subsection("Part 2: Hermite error vs most accurate Hermite (nsteps=$(h_nsteps[end]))")
    print_error_vs_ref("Hermite", h_nsteps, h_finals, h_ref, order)

    print_subsection("Part 3: Filon(w=0) error vs most accurate Filon(w=0) (nsteps=$(f0_nsteps[end]))")
    print_error_vs_ref("Filon(w=0)", f0_nsteps, f0_finals, f0_ref, order)

    print_subsection("Part 4a: Filon(w=0) error vs best Hermite (nsteps=$(h_nsteps[end]))")
    print_error_vs_ref("Filon(w=0)", f0_nsteps, f0_finals, h_ref, order)

    print_subsection("Part 4b: Hermite error vs best Filon(w=0) (nsteps=$(f0_nsteps[end]))")
    print_error_vs_ref("Hermite", h_nsteps, h_finals, f0_ref, order)
    ...
    println(@sprintf("    |Hermite_best - Filon(w=0)_best| = %.6e", norm(h_ref - f0_ref)))
    println(@sprintf("    |Hermite_best - Filon(rot)_best| = %.6e", norm(h_ref - fr_ref)))
    println(@sprintf("    |Filon(w=0)_best - Filon(rot)_best| = %.6e", norm(f0_ref - fr_ref)))
end
=#
...
# Helper: compute Richardson error vector from a list of final states
function richardson_errors(finals, order)
    return [norm(finals[i+1] - finals[i]) * 2^order / (2^order - 1)
            for i in 1:length(finals)-1]
end
...
save(joinpath(@__DIR__, "..", "Plots", "control_functions.png"), fig1)
save(joinpath(@__DIR__, "..", "Plots", "control_functions.pdf"), fig1)
println("\nSaved: Plots/control_functions.{png,pdf}")
...
ns_fine = 2^max_power
rot_prob.nsteps = ns_fine
hist_fine = eval_forward(
    QuantumGateDesign.VectorSchrodingerProb(rot_prob, initial_cond),
    rot_controls, pcof, order=order_finest)
...
save(joinpath(@__DIR__, "..", "Plots", "state_evolution.png"), fig2)
save(joinpath(@__DIR__, "..", "Plots", "state_evolution.pdf"), fig2)
println("Saved: Plots/state_evolution.{png,pdf}")
...
save(joinpath(@__DIR__, "..", "Plots", "convergence_richardson.png"), fig3)
save(joinpath(@__DIR__, "..", "Plots", "convergence_richardson.pdf"), fig3)
println("Saved: Plots/convergence_richardson.{png,pdf}")
...
save(joinpath(@__DIR__, "..", "Plots", "cnot3_combined.png"), fig4)
save(joinpath(@__DIR__, "..", "Plots", "cnot3_combined.pdf"), fig4)
println("Saved: Plots/cnot3_combined.{png,pdf}")
