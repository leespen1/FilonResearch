...
U0 = rot_prob.u0 + im.*rot_prob.v0

U0 .= 1 # For equally-distributed initial condition, get some high-frequency stuff
rot_prob.u0 .= real(U0)
rot_prob.v0 .= imag(U0)
...
for s in s_values
    ...
    for k in min_power:max_power
        ...
        # Cross-method difference (L2-in-time, same nsteps)
        cross_str = fmt(l2_time_diff(h_hists[end], f0_hists[end], Tmax))

        # Norms of final state vectors
        norm_h_str  = @sprintf("%12.2e", norm(h_hists[end][:,end]))
        norm_f0_str = @sprintf("%12.2e", norm(f0_hists[end][:,end]))
        norm_fr_str = @sprintf("%12.2e", norm(fr_hists[end][:,end]))
